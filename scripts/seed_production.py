#!/usr/bin/env python3
"""
Seed production MongoDB database
Usage: python seed_production.py
Make sure to set MONGO_URL and DB_NAME environment variables for production
"""
...
async def seed_production_database():
    # Get production MongoDB credentials from environment variables
    mongo_url = os.environ.get('MONGO_URL')
    db_name = os.environ.get('DB_NAME')
    
    if not mongo_url:
        print("❌ Error: MONGO_URL environment variable is required")
        print("Example: export MONGO_URL='mongodb+srv://user:pass@cluster.mongodb.net/'")
        sys.exit(1)
    
    if not db_name:
        print("❌ Error: DB_NAME environment variable is required")
        print("Example: export DB_NAME='rental_management_prod'")
        sys.exit(1)
    ...
    visit_requests = [
        {
            ...
            "activity_log": [
                ...
                {
                    "action": "Assigned to agent: Emily Rodriguez",
                    "timestamp": datetime.now(timezone.utc).isoformat(),
                    "actor": "System",
                    "actor_role": "admin"
                }
        ...
        },
        {
            ...
            "activity_log": [
                ...
                {
                    "action": "Assigned to agent: David Kumar",
                    "timestamp": (datetime.now(timezone.utc) - timedelta(days=1)).isoformat(),
                    "actor": "System",
                    "actor_role": "admin"
                },
                {
                    "action": "Stage changed from 'new' to 'visit_scheduled'",
                    "timestamp": datetime.now(timezone.utc).isoformat(),
                    "actor": "David Kumar",
                    "actor_role": "agent"
                }
    ...
    print(f"   • Visit Requests: {len(visit_requests)}")
    print(f"   • Rental Records: {len(rental_records)}")
    print("="*50)
